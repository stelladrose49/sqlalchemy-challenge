...
# Import Python-SQL toolkit and ORM
...
def precipitation():
    ...
    results= results = session.query(Measurement.date, Measurement.prcp).filter(Measurement.date >= last_date).order_by(Measurement.date)

    prec_dict = dict(results)

    print(f"Results of Precipitation - {prec_dict}")
